...
### MODIFIED BY RAJAT: split-scp takes values from 0...$nj-1 but
### JOB takes values from 1...$nj, so I am using these sequences,
### but moving the scps 1 number forward.
...
for (( n=0; n<$nj; n++ )); do
    utils/split_scp.pl -j $nj $n $data/norm_feats.scp $data/split$nj/scp.$n
done

for (( n=$nj; n>0; n-- )); do
    prev=$(($n - 1))
    mv $data/split$nj/scp.$prev $data/split$nj/scp.$n
done
...
$cmd JOB=1:$nj $logdir/splice_mfcc_${name}.JOB.log \
  splice-feats --left-context=$splice_ctx --right-context=$splice_ctx \
   scp:$data/split$nj/scp.JOB \
   ark,scp,t:$mfccdir/splice_mfcc_${name}.JOB.ark,$mfccdir/splice_mfcc_${name}.JOB.scp \
    || exit 1;


# concatenate the .scp files together.
for n in $(seq $nj); do
  cat $mfccdir/splice_mfcc_$name.$n.scp || exit 1;
done > $data/spliced_feats.scp || exit 1
