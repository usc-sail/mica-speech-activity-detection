...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    ...
    compute-mfcc-feats $vtln_opts --verbose=2 --config=$mfcc_config ark,p:- ark,p:- \| \
    copy-feats --compress=$compress $write_num_frames_opt ark,p:- \
      ark,scp,p:$mfccdir/raw_mfcc_$name.JOB.ark,$mfccdir/raw_mfcc_$name.JOB.scp \
     || exit 1;
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    compute-mfcc-feats  $vtln_opts --verbose=2 --config=$mfcc_config \
     scp,p:$logdir/wav_${name}.JOB.scp ark:- \| \
      ...
      ark,scp,t:$mfccdir/raw_mfcc_$name.JOB.ark,$mfccdir/raw_mfcc_$name.JOB.scp \
      || exit 1;
fi
...
###
### COMMENTED BY RAJAT 08/31/17, since utt2spk file is not being used.
###
...
END=$(date +%s)
DIFF=$((END-$START))
echo "make_mfcc.sh took $DIFF sec"
