        ...
        while(countl<len(lines)):
            if re.match("\d+:\d+:\d+,\d+ --> \d+:\d+:\d+,\d+", lines[countl]):
                ...
                if uttline!='':
                    ...
                    if((uttline[0]=='(' and uttline[-1]==')')):   ## used to remove music, left out for now-->  or re.search("\xe2",uttline) ):
                        x1[countu][2] = 0
                else:
                    x1[countu][2] = 0
                countu += 1
...
srtpath = open("../srt_wav_Paths/srt_files.txt","r").readlines()
destpath = '/proj/rajat/temp_ctm/'
buf = 0
thr = 0
