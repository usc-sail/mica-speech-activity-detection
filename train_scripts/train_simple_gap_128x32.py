...
exp_dir = proj_dir + 'exp_data/exp5.3/'
...
def enqueue_speech(sess):
    ...
    while True:
        ...
        for i in range(0,QUEUE_BATCH,4):
            data = INPUT_DATA.next()
            for j in range(4):
                fts[i+j] = data[1][:,j*32:(j+1)*32]
                tgts[i+j] = data[2][j*32:(j+1)*32]
        COUNTSP+=1
